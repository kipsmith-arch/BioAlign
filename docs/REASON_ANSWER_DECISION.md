...
项目采用 Continue-Pretraining → SFT → DPO 三阶段流水线。每个阶段的目标有本质区别：

| 阶段 | 核心目标 | 数据形态 | 监督信号 | 学到什么 |
|------|---------|---------|---------|---------|
| **Stage 1：领域继续预训练** | 领域知识注入 | 23.6 万条裸序列（DNA/RNA/Protein） | next-token prediction（无监督） | 序列表征：motif 模式、碱基统计、功能关联先验 |
| **Stage 2：指令微调** | 任务理解 + 答案生成 | 28.9 万条 QA 对（input → output） | 只对 assistant 部分算 loss | 看到分类 prompt 走分类分支；看到数值 prompt 输出数字 |
| **Stage 3：DPO 偏好对齐** | 答案选择偏好 | 2.5 万对 (chosen, rejected) | DPO loss（chosen log prob > rejected） | 在多个合理答案中偏好某种风格（如结构化 + 简洁） |

### 8.2 三个阶段都在"改变模型的说话方式"，但层次不同

| 阶段 | 改变了什么 | 没改变什么 |
|------|-----------|-----------|
| Stage 1 | 模型**词汇层面**：看到 ATCG 会想"这是 DNA"，看到 MAFFT 会想"这是蛋白比对" | 不会用句子回答——还是 next-token 续写 |
| Stage 2 | 模型**句法层面**：看到"请判断..."会用句子回答；看到分类问题会给出 binary 答案 | 不会评估答案好坏——给什么学什么 |
| Stage 3 | 模型**风格层面**：在多种合理答案中偏好某种（如简洁 + 带标签） | 不会拒绝回答——DPO 只会调整概率 |

### 8.3 本次项目踩坑的核心教训：Stage 3 退化的本质

**现象**：Stage 3（DPO）训练后，多个任务指标反而下降：

- `Protein/Thermostability` spearman：s1_s2 = 0.49 → stage3 = 0.13（-0.36）
- `DNA/emp` MCC：s1_s2 = 0.00 → stage3 = -0.96（-0.96）
- `enhancer_activity hk_PCC`：s1_s2 = 0.11 → stage3 = -0.05
...
```
原始设计：
  Stage 2 SFT 数据 output：自然语言描述（如"The interaction is not predicted..."）
  Stage 3 DPO chosen：     同一个自然语言描述
  Stage 3 DPO rejected：   Stage 2 采样输出（另一个自然语言描述）
  eval parser 期望：       关键词（positive / negative / 数字）
```

**问题链条**：

1. Stage 2 让模型学会写自然语言描述
2. DPO 让模型更擅长写自然语言描述（偏好 chosen 的句式）
3. eval parser 只看关键词，从自然语言里提取出来的预测经常是反的或随机的
4. **DPO 不是让模型变好，是让模型更坚定地走向与 eval 不兼容的方向**

### 8.4 修复原则：三阶段目标必须对齐到最终使用场景

> **下游是 parser → 三阶段都应该向"输出可解析的格式"对齐**

本项目三个阶段都应以下游使用场景为准则：

| 下游场景 | Stage 1 学什么 | Stage 2 学什么 | Stage 3 学什么 |
|---------|---------------|---------------|---------------|
| **本项目（parser 提取）** | 生物序列表征 | 任务格式 + 输出 label | **偏好输出 label 而不是自由文** |
| **助手产品（聊天）** | 通用知识 | 自然语言回答 + 推理 | 偏好有帮助、诚实、无害的回答 |
| **代码生成** | 编程语言模式 | 任务理解 + 代码生成 | 偏好可运行、风格一致的代码 |

**修复方法**（本文档已实施）：

1. Stage 2 训练数据 output 从"自然语言描述"改为 `<reason>...</reason>\n<ans>label</ans>` 结构化格式
2. Stage 3 DPO chosen = 结构化格式输出，rejected = Stage 2 采样的自然语言
3. eval parser 从 `<ans>` 块提取 label，fallback 到关键词
4. **三阶段现在都向"输出可被 parser 提取的格式"对齐**

### 8.5 一句话总结

> **Stage 1 学"知识"，Stage 2 学"做任务"，Stage 3 学"做对任务"。三者递进，但目标必须对齐到最终使用场景。**

如果下游是聊天助手，三个阶段都应偏向"自然、友好"；如果下游是 parser，三个阶段都应偏向"结构化、可提取"。**偏离了下游场景的训练，等于在错误的轨道上越走越快**。
